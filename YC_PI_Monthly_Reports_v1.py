...
def find_project_number_column(columns):
    """
    Robust Project Number finder. Accepts variations like:
      - Project Number
      - Project #
      - Project ID
      - Project No
    """
    # First: try exact/keywords
    try:
        return find_column_by_exact_or_keywords(columns, PROJECT_COL_NAME, keywords=["project", "number"])
    except KeyError:
        pass

    # Second: heuristic candidates
    candidates = []
    for c in columns:
        low = c.lower()
...
def compute_org7(value) -> str:
    """
    Use the FIRST 7 alphanumeric characters found in the value to designate folder.
    Example:
      'LPSC001 - Dept of X' -> 'LPSC001'
    """
...
def style_sheet(workbook, sheet_name, currency_cols, footnote_text, hide_indirect=True):
    """
    Style output sheet and add footnote.
    """
    ws = workbook[sheet_name]
...
def process_workbooks_bytes(master_bytes, award_bytes_list, date_pulled="", pi_filter="", org7_filter=""):
    ...
    # Identify key columns (robust project col)
    ...
    # Defensive check: make sure the detected project col survived
    if project_col not in df_active.columns:
        raise KeyError(
            f"Detected project_col='{project_col}', but it is not present after column selection.\n"
            f"df_active columns: {list(df_active.columns)}"
        )
    ...
    # Footnote text (single indirect rate? then show it)
...
def main():
    ...
    st.markdown(
        ...
        Output ZIP is organized by **Project Owning Organization (first 7 alphanumeric chars)**:
